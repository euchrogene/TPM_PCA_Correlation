...
- Colours in the ordination follow the Okabe-Ito palette and are paired with distinct marker shapes, so the groups stay separable in greyscale and for colour-vision-deficient readers. `-palette tab20` is available when more than ten groups have to be told apart.
...
- The frozen wrapper keeps its module constants inside the compressed archive, so `strings` on the executable reveals neither the signing key nor the host marker path nor the option names. That depends on `optimize=0` in the spec; at `optimize=2` PyInstaller stores the entry script uncompressed and prints all three in plain text. The spec says so at the setting, and a build that changes it should be checked with `strings` before it ships.
- Every run is handed a freshly signed, short-lived token that the worker checks against the key baked into the image. The token travels in the container environment and is never written into the results folder, so `run_container.sh` and `RUN_SPEC.json` stay safe to share. Rebuilding the image with a new key requires reinstalling the matching wrapper.
- A deployed wrapper is a compiled executable, so it cannot be read or edited on a customer machine. Debugging works from the container side instead: `docker run --rm --entrypoint cat managene7/tpm-pca-correlation:v1.0 /opt/euchrogene/tpm_pca_correlation_container.py` prints the worker, and fixes go into this repository followed by a rebuild and a reinstall.
- The Methods paragraph inside `report.html` is generated from the settings of the run that produced it and from tool versions queried at run time, ready to copy into a manuscript.
